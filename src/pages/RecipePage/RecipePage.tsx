...
const complexityOrder: Record<string, number> = { Легко: 1, Помірно: 2, Складно: 3 };
...
interface Collection { id: string; name: string; recipes: { id: string; dateSaved: string }[] }
...
const RecipesPage: React.FC = () => {
  ...
  const fetchRecipes = async () => {
    try {
      setLoading(true);
      if (!isAuthenticated) {
        // Если ещё не авторизованы, вызываем login через форму (или можно автоматом тестово)
        await login("admin", "password123");
      }
      const data = await getRecipes();
      if (data?.length) setAllRecipes(data);
    } catch (err) {
  ...
  const sortedRecipes = useMemo(
    () => [...allRecipes].sort((a, b) => {
      switch (sortBy) {
        ...
        case 'time': return parseTime(a.time) - parseTime(b.time);
        case 'complexity': return (complexityOrder[a.complexity] ?? 99) - (complexityOrder[b.complexity] ?? 99);
        case 'newest': return parseInt(b.id.replace(/\D/g, ''), 10) - parseInt(a.id.replace(/\D/g, ''), 10);
      ...
      <div className={styles.mainBlock}>
        ...
        <div className={styles.headerBlock}>
          <div className={styles.headerButtonBlock}>
            ...
            <button className={styles.filterButton} onClick={() => setFilterModalOpen(true)}>
              Фільтр <img src={iconFilter} alt="filter" />
            </button>
          </div>
        </div>
        ...
        {filters.cuisines?.length || filters.category || filters.time || filters.complexity || filters.diet ? (
          <div className={styles.selectedFilters}>
            <button className={styles.clearFiltersButton} onClick={() => setFilters({ cuisines: [], category: '', time: '', complexity: '', diet: '' })}>Очистити</button>
            {filters.cuisines?.map((c: string) => <span key={c} className={styles.filterTag}>{c}</span>)}
        ...
        <FilterModal isOpen={filterModalOpen} onClose={() => setFilterModalOpen(false)} onApply={(f) => { setFilters(f); setFilterModalOpen(false); }} />
      </div>
    </main>
  );
};
